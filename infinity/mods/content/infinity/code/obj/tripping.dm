...
/obj/Crossed(mob/living/carbon/human/H)
	. = ..()
	if(trip_chance && prob(trip_chance) && trip_check(H))
		H.trip_act(src)

/obj/proc/trip_check(mob/user)
...
/obj/structure/cable/trip_check(mob/user)
	var/turf/T = src.loc
	if(!T.is_plating())
		return FALSE
...
/obj/structure/disposalpipe/trip_check(mob/user)
	var/turf/T = src.loc
	if(!T.is_plating())
		return FALSE
	if(dpdir & user.dir)
