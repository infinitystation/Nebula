/decl/webhook/message_ban
	id = "webhook_message_ban"

/decl/webhook/message_ban/get_message(var/list/data)
	. = ..()
	...
	if(!data["banned"])	
		data["banned"] = "ДАННЫЕ УДАЛЕНЫ"

	if(!data["reason"])	
		data["reason"] = "ДАННЫЕ УДАЛЕНЫ"
