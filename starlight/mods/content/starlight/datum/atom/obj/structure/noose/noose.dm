...
/obj/structure/noose
	...
	icon_state = "noose"
	icon = 'starlight/mods/content/starlight/datum/atom/obj/structure/noose/noose.dmi'
	anchored = TRUE
	can_buckle = TRUE
	buckle_lying = FALSE
	layer = 5
...
/obj/structure/noose/attackby(obj/item/W, mob/user, params)
	if(W.edge)
		user.visible_message(\
			SPAN_NOTICE("\The [user] cuts the noose."),\
	...
	if(M == user)
		...
		if(do_after(user, 5 SECONDS))
			if(buckle_mob(M))
				...
				playsound(user, 'starlight/mods/content/starlight/datum/atom/obj/structure/noose/noose_buckle.ogg', 50, 1, -1)
				return TRUE
...
/obj/structure/noose/Process()
	...
	switch(ticks)
		...
		if(3)
			...
			if(buckled_mob)
				playsound(buckled_mob, 'starlight/mods/content/starlight/datum/atom/obj/structure/noose/noose_idle.ogg', 50, 1, -3)
