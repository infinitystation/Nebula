...
/obj/structure/noose
	...
	icon_state = "noose"
	icon = 'mods_infinity/content/starlight/datum/atom/obj/structure/noose/noose.dmi'
	anchored = TRUE
	can_buckle = TRUE
	buckle_lying = FALSE
	layer = 5
	...
	if(M == user)
		...
		if(do_after(user, 5 SECONDS))
			if(buckle_mob(M))
				...
				playsound(user, 'mods_infinity/content/starlight/datum/atom/obj/structure/noose/noose_buckle.ogg', 50, 1, -1)
				return TRUE
...
/obj/structure/noose/Process()
	...
	switch(ticks)
		...
		if(3)
			...
			if(buckled_mob)
				playsound(buckled_mob, 'mods_infinity/content/starlight/datum/atom/obj/structure/noose/noose_idle.ogg', 50, 1, -3)
